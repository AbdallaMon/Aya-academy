// ===========================================================================
// subscriptionScheduler — end-of-month cron for usage-based subscription billing.
//
// node-cron has no "last day of month" token, so we schedule on the candidate
// late days (28-31) at 23:00 and fire the billing run ONLY on the actual last
// day of that month (detected via "tomorrow is the 1st"). This is correct for
// every month length — 28 (Feb), 29 (Feb leap), 30, and 31 — without
// special-casing.
//
// startSubscriptionScheduler(): called on server boot (guarded — never breaks
// boot). It invokes subscriptionUsecase.generateMonthlyUsageInvoices(), which
// freezes each active student's consumed hours into a USAGE subscription +
// demand invoice for the coming month. The handler swallows errors so a failed
// run never crashes the cron.
// ===========================================================================
...
export function startSubscriptionScheduler() {
  ...
  task = cron.schedule(CRON_EXPR, () => {
    ...
    subscriptionUsecase.generateMonthlyUsageInvoices(now).catch((err) => {
      ...
      console.error(
        "[subscription-cron] usage-billing error:",
  ...
  // eslint-disable-next-line no-console
  console.log(
    "[subscription-cron] end-of-month usage billing scheduled (23:00 on the last day of each month)",
  );
}
