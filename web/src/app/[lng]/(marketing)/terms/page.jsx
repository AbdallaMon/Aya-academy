...
export async function generateMetadata({ params }) {
  ...
  const content = getLegalContent('terms', lng);
...
export default async function TermsRoute({ params }) {
  const { lng } = await params;
    ...
    <>
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: content.title, url: absolute('/terms') },
        ])}
      />
      <LegalPage type="terms" lng={language} />
    </>
  );
}
